<template>
  <div style="width:2000px;height:1000px">
    <div>
      ...
      <div style="height:120px;border:0;margin:0;width:2%;float:left">
        <div style="height:60px;">
          <img src="../assets/箭头_左.png" style="width:20px;height:20px;margin-top:50px" @click="back"/>
        </div>
    ...
    <div style=" height: 100%;width: 60%;margin:0;padding:0;border:0;float:left;">
      ...
      <!--      <input class='inputsurname' v-model="surname" placeholder="真实姓名"/>-->
      <br>
      <i class="point" style="margin-left:250px;margin-right:10px"/>
      <input class="inputgivenname" v-model="name" placeholder="真实姓名"/>
      <div>
        <i class="point" style="margin-left:250px;margin-right:10px"/>
        <el-radio v-model="gender" label="male" style="margin-left:0;margin-top:10px;margin-bottom:10px" size="medium">
          男
        </el-radio>
      ...
      <div>
        <i class="point" style="margin-left:250px;margin-right:10px"/>
        <el-date-picker type="date" placeholder="选择出生日期" style="margin-left:0px;margin-top:30px;margin-bottom:30px"
                        v-model="birthday"></el-date-picker>
      </div>
      <div>
        <i class="point" style="margin-left:250px;margin-right:10px"/>
    ...
    <div style=" height: 100%;width: 40%;margin:0;padding:0;border:0;float: left;">
      <div class="curved_box" style="margin-top:30px;margin-left:0px;margin-right:20px;width:400px;height:700px">
        <p style="font-size:25px;margin-left:20px;font-weight:200;margin-bottom:0">{{ departcity }}
          ({{ departairport }}) 至</p>
        <br>
        ...
        <div>
          <div style="width:48%;float:left;margin-left:0px">
            <p style="font-size:20px;margin-left:20px;margin-right:0;font-weight:10;margin-top:0px;display:inline">
              {{ departtime }} {{ departairport }}</p>
          </div>
          <div style="width:4%;float:left">
            <hr
              style="border:0;background-color:#021B41;height:1px;margin-top:10px;width:70%;margin-left:8px;margin-right:0">
          </div>
          <div style="width:48%;float:left">
            <p style="font-size:20px;margin-left:10px;font-weight:10;margin-top:0px;display:inline">{{ destime }}
              {{ desairport }}</p>
          </div>
        </div>
        <br>
        <p style="font-size:15px;margin-left:20px;font-weight:400;margin-top:50px;margin-bottom:50px">
          {{ seatType }}托运行李</p>
...
export default {
  ...
  methods: {
    ...
    toafterBuy: function () {
      ...
      formData.append("real_name", this.name)
      formData.append("Id_number", this.id)
      formData.append("money", this.price)
      formData.append("flight_number", this.company)
      formData.append("date", this.date)
      this.$http.post('api/book_ticket/', formData).then(result => {
        if (result.data.status === 0) {
          if (tag === true) {
            ...
            this.$router.push(
              {
                path: "/afterBuy", query: {
                  desCity: this.descity,
                  departCity: this.departcity,
                  departAirport: this.departairport,
                  desAirport: this.desairport,
                  departTime: this.departtime,
                  desTime: this.destime,
                  price: this.price,
                  time: this.time,
                  flight_number: this.company,
                }
              })
          }
        }else{
          this.$alert(result.data.msg, '信息不正确', {
